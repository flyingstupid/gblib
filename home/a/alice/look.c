...
int render_exits()
{
    ...
    switch(sizeof(exits))
    {
        ...
        case 1:
        {
            write(CYN + "The only obvious exit is " + HIY + exits[0] + NOR + CYN
+ "." + NOR + "\n\n"); 
            break;
        }
        default:
...
int render_room()
{
    string *users, *objects;
    
    if(!objectp(room)) 
    {
        return notify_fail("You see nothing because you have no
environment!\n");
    }
...
:));
    ...
    if(sizeof(users) > 0)
    {
        write(implode(map(users, (: capitalize($1->query_short() +
$1->GetPositionPost()) :)), "\n") + "\n");
    }
    if(sizeof(objects) > 0)
    {
        write(implode(map(objects->query_short(), (: capitalize($1) :)), "\n") +
"\n");
        //write(implode(map(objects, (: file_name( $1 ) :)), "\n") + "\n");
    }
    ...
    if(living(ob))
    {
        ...
        else
        {
            ...
            tell_room(room, this_player()->query_cap_name() + " looks at " +
                ob->query_cap_name() + ".\n", ({ this_player(), ob }) );
            tell_object(ob, this_player()->query_cap_name() + " looks at
you.\n");
