...
int main(string arg)
{
    ...
    if(!ob)
    {
        return(notify_fail("The object '" + arg + "' can not be found.\n"));
    }

    if(ob->query("prevent_get") || ob->prevent_get())
    {
        return(notify_fail("The object '" + arg + "' can not be picked up.\n"));
    }

    if( is_a( SAMPLE, ob ) )
    {
	    return(notify_fail( ob->query_short() + " must be collected.\n"));
    }

    if(!ob->move(this_player()))
    {
        return(notify_fail("Unable to get '" + arg + "'.\n"));
    }
