...
int main(string args)
{
     ...
     if(!adminp(this_user())) return notify_fail("Error [redev]: Access
denied.\n");
...
online.\n"));
     if(!devp(query_privs(user))) return(notify_fail("Error [makedev]: That user
is not a developer.\n"));
...
"'.\n");
     ...
     write("Success [revdev]: User '" + capitalize(user->query_name()) + "' is
