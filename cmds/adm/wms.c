...
int main(string str)
{
     if(!adminp(this_user())) return notify_fail("Error [wms]: Access
denied.\n");
...
the user you wish to add [quit]: " + NOR);
          count = 1;
          input_to("add_user2");
          return;
     }
...
user you wish to add [quit]: " + NOR);
     ...
     if(users[str])
     {
          write(RED + "Error: " + NOR + HIW + "User already in database...
Aborting.\n" + NOR);
          return;
     }

     data += (["history" : ({"User created -- User Management - " +
capitalize(this_player()->query_name()) + " - " + (string)ctime(time()) }) ]);
     data += (["notes" : ({}) ]);
     data += (["flags" : ({}) ]);
     data += (["profile" : ""]);
     data += (["added" : time() ]);
...
NOR);
     ...
     current_file = "/tmp/" + random(9999999) + "." +
...
"." + this_player()->query_name();
     ...
     log_file(WMS_LOG, capitalize(str) + " was added to the WMS database
manually by " + capitalize(this_player()->query_name()) + " on " + ctime(time())
+ "\n");
     return;
}
...
void add_user3()
{
     ...
     rm(current_file);
     current_file = "";
     write(HIG + "Success: " + NOR + HIW + "User addition to database
complete.\n" + NOR);
...
void edit_user2(string name)
{
     ...
     if(!name)
     {
          ...
          write(RED + "Error: " + NOR + HIW + "No name supplied.\nPlease enter
the user you wish to edit [quit]: ");
          count = 1;
          input_to("edit_user2");
          return;
     }
    ...
    if(!users[name])
    {
          write(RED + "Error: " + NOR + HIW + "User not found in
...
"." + this_player()->query_name();
     ...
     log_file(WMS_LOG, capitalize(this_player()->query_name()) + " edited " +
capitalize(name) + "'s profile on " + ctime(time()) + ".\n");
     return;
}
...
void edit_user3()
{
     ...
     users[current]["history"] += ({ "Profile edited -- User management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
...
void note_user()
{
     ...
     write(HIY + "Welcome to WMS...\n\n" + NOR);
     write(HIW + "Please enter the name of the user who you wish to add a note
to: " + NOR);
     input_to("note_user2");
     return;
}
...
void note_user2(string name)
{
     ...
     if(!name)
     {
          ...
          write(RED + "Error: " + NOR + HIW + "No name supplied.\nPlease enter
the user you wish to note [quit]: ");
          count = 1;
          input_to("note_user2");
          return;
     }
...
the type of note you wish to add [quit]: ");
          count = 1;
          input_to("note_user3");
          return;
     }
...
enter the type of note you wish to add [quit]: ");
          count = 1;
     ...
     switch(to_int(str))
     {
          case 1..9 : break;
          default : {
               write(RED + "Error: " + NOR + HIW + "Invalid type
supplied.\nPlease enter the type of note you wish to add [quit]: ");
      ...
      write(HIW + "Is this note to be public (viewable by other admins)? [yes] :
" + NOR);
...
void note_user4(string str, int i)
{
     ...
     new_note += (["perm" : str, "type" : type, "admin" :
this_player()->query("name"), "date" : time(), "for" : current]);
     note += ([idn : new_note]);
...
"." + this_player()->query_name();
     ...
     log_file(WMS_LOG, capitalize(this_player()->query_name()) + " made a " +
type + " note for " + capitalize(current) + " on " + ctime(time()) + ".\n");
     return;
}
...
void note_user5()
{
     ...
     user = users[current];
     arr = user["history"];
     arr += ({ "Note added -- Note Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
     user["history"] = arr;
     users[current] = user;
     current = "";
...
void flag_user()
{
     ...
     write(HIY + "Welcome to WMS...\n\n" + NOR);
     write(HIW + "Please enter the name of the user who you wish to flag: " +
NOR);
     input_to("flag_user2");
     return;
}
...
void flag_user2(string str)
{
     ...
     if(!str)
     {
          ...
          write(RED + "Error: " + NOR + HIW + "No name supplied.\nPlease enter
the type of note you wish to add [quit]: " + NOR);
          count = 1;
          input_to("flag_user2");
          str = "quit";
          return;
...
database.\nPlease enter the type of note you wish to add [quit]: " + NOR);
     ...
     data = users[str];
     current = str;
     write(HIW + capitalize(str) + " currently has flags toggled as
follows:\n\n" + NOR);
     flags = data["flags"];

     if(member_array("excellent performance", flags) != -1) write(HIW + "1 )
Excellent performance : Toggled\n" + NOR);
     else write ("1 ) Excellent performance : Not Toggled\n");
     if(member_array("inactive", flags) != -1) write (HIW + "2 ) Inactive :
Toggled.\n" + NOR);
     else write("2 ) Inactive : Not Toggled.\n" + NOR);
     if(member_array("suspended", flags) != -1) write(HIW + "3 ) Suspended :
Toggled.\n" + NOR);
     else write("3 ) Suspended : Not Toggled.\n" + NOR);
     if(member_array("discipline problem", flags) != -1) write(HIW + "4 )
Discipline Problem: Toggled.\n" + NOR);
     else write("4 ) Discipline Problem: Not Toggled.\n" + NOR);
     if(member_array("recruiter", flags) != -1) write(HIW + "5 ) Recruiter :
Toggled.\n" + NOR);
     else write("5 ) Recruiter : Not Toggled.\n" + NOR);

     write("\n");
     write(HIW + "Please enter the number of the item you wish to
toggle/untoggle [quit]: " + NOR);
     input_to("flag_user3");
     return;
}
...
supplied.\nPlease enter the number of the item you wish to toggle/untoggle
[quit]: " + NOR);
          input_to("flag_user3");
          count = 1;
          return;
     ...
     switch(to_int(str))
     {
          case 1 : {
               ...
               if(member_array("excellent performance", flags) != -1)
               {
                    ...
                    hist += ({"Untoggled E -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
               ...
               {
                    flags += ({ "excellent performance" });
                    hist += ({"Toggled E -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
          }
          ...
          case 2 : {
               ...
               {
                    flags -= ({ "inactive" });
                    hist += ({"Untoggled I -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
               ...
               {
                    flags += ({ "inactive" });
                    hist += ({"Toggled I -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
          }
          ...
          case 3 : {
               ...
               {
                    flags -= ({ "suspended" });
                    hist += ({"Untoggled S -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
...
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
          ...
          case 4 : {
               ...
               if(member_array("discipline problem", flags) != -1)
               {
                    ...
                    hist += ({"Untoggled D -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
               ...
               {
                    flags += ({"discipline problem"});
                    hist += ({"Toggled D -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
          }
          ...
          case 5 : {
               ...
               {
                    flags -= ({"recruiter"});
                    hist += ({"Untoggled R -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
               ...
               {
                    flags += ({"recruiter"});
                    hist += ({"Toggled R -- Flag Management - " +
capitalize(this_player()->query_name()) + " - " + ctime(time()) });
                    break;
               }
          }
          ...
          default : {
               ...
               write(HIR + "Error : " + NOR + HIW + "Invalid argument.\nPlease
enter the number of the item you wish to toggle/untoggle [quit]: " + NOR);
               count = 1;
               input_to("flag_user3");
               return;
          }
     ...
     log_file(WMS_LOG, capitalize(this_player()->query_name()) + " modified " +
capitalize(current) + "'s flags on " + ctime(time()) + "\n");
     current = "";
     save_object(datafile());
     return;
}
...
exiting WMS...\n" + NOR);
     ...
     switch(to_int(str))
     {
          ...
          default : {
               write(RED + "Error: " + NOR + HIW + "Invalid argument
passed.\n\n" + NOR);
               filter_list();
               return;
          }
...
"# of Notes"));
     ...
     for(i = 0; i < sizeof(keys); i ++)
     {
          ...
          printf("%-10s %s -- %s -- %d\n", capitalize(keys[i]), " :: " +
ctime(data["added"]), flag, sizeof(data["notes"]));
     }
...
void remove_user()
{
     ...
     write(HIY + "Welcome to WMS...\n\n");
     write(HIR + "Note: You may not undo this action. There is no
confirmation.\n");
     write(HIW + "Please enter the name of the user you wish to remove from the
database: ");
...
Aborting.\n" + NOR);
     ...
     log_file(WMS_LOG, capitalize(str) + " was deleted manually from the WMS
database by " + capitalize(this_player()->query_name()) + " on " + ctime(time())
+ ".\n");
...
id of the note you wish to read [quit]: ");
          count = 1;
          input_to("read_note2");
          return;
     }
...
enter the id of the note you wish to read [quit]: ");
          count = 1;
          input_to("read_note2");
          return;
     }
     ...
     if(!notes[note_id])
     {
          write(RED + "Error: " + NOR + HIW + "Note not found in database... Now
exiting WMS.\n" + NOR);
          return;
     }
...
exiting WMS.\n" + NOR);
     ...
     if(!data["msg"])
     {
           write(RED + "Error: " + NOR + HIW + "No Content in Note... Now
exiting WMS.\n" + NOR);
           return;
     }
...
NOR);
     ...
     for(i = 0; i < sizeof(note_ids); i++)
     {
          ...
          if(note["perm"] != "public" && note["perm"] !=
this_player()->query("name")) continue;
          write(HIG + " #" + note_ids[i] + " - " + note["type"] + " - " +
capitalize(note["admin"]) + "\n" + NOR);
     }
...
"# of Notes"));
     ...
     for(i = 0; i < sizeof(keys); i ++)
     {
          ...
          printf("%-10s %s -- %s -- %d\n", capitalize(keys[i]), " :: " +
ctime(data["added"]), flags, sizeof(data["notes"]));
     }
}
...
{
     return("SYNTAX: wms
[<username>||add||remove||list||note||read||edit||flag]\n\n"
     "Wms is a wizard management system that allows admins to quickly and
easily\n"
     "manage their wizards (staff). To make use of the wms command, you must\n"
     "have adminaccess or above.\n"
     "If you do not supply any argument, wms will return a summary of all
users\n"
     "in the wms database. If you supply a username, it will return the data
on\n"
     ...
     "you to add a note to a user. The read arugment allows you to read
notes.\n"
