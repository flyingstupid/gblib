...
int main(string arg)
{
     ...
     if(!adminp(this_user())) return notify_fail("Error [reboot]: Access denied.\n");
     ...
     if(arg == "stop")
     {
          if(!status) return notify_fail("Error: There is no shutdown or reboot
currently in progress.\n");
          else SHUTDOWN_D->stop();
          log_file(LOG_SHUTDOWN, capitalize(this_player()->query_name()) + "
canceled the sequence (" + time + "m) on " + ctime(time()) + "\n");
          return 1;
     }
     ...
     else
     {
          ...
          if(time == 0 && arg != "now" && arg != "0") return
notify_fail("SYNTAX: shutdown [<stop>||<time>/now]\n");
          log_file(LOG_SHUTDOWN, capitalize(this_player()->query_name()) + "
started reboot sequence (" + time + "m) on " + ctime(time()) + "\n");
