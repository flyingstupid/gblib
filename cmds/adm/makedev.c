...
int main(string args)
{
     ...
     if(!adminp(this_user())) return notify_fail("Error [makedev]: Access
denied.\n");
     ...
     if(!user) return(notify_fail("Error [makedev]: User '" + args + "' is not
online.\n"));
...
"/private");
     ...
     write_file("/doc/journals/journal." + user->query_name(),
        capitalize(user->query_name()) + "'s dev journal (Created: " +
ctime(time()) + ")\n\n");
     catch(link("/doc/journals/journal." + user->query_name(),
        "/home/" + user->query_name()[0..0] + "/" + user->query_name() +
"/private/journal."
        + user->query_name()));
     user->add_path("/cmds/wiz/");
     user->add_path("/cmds/object/");
     ...
     write("Success [makedev]: User '" + capitalize(user->query_name()) + "' now
has developer status.\n");
