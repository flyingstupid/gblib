...
{
     if(!adminp(this_user())) return notify_fail("Error [nuke]: Access
denied.\n");
...
void confirm_nuke(string str, string user)
{
     ...
     if(str == "y" || str == "yes")
     {
          write("\nWarning [nuke]: Now stripping user of system group
memberships.\n");
          ...
          {
               if(isMember(user, group)) write("\t* " + group + " membership
revoked.\n");
...
"'.\n");
               ...
               if(environment(body)) tell_room(environment(body), "You watch as
" + capitalize(user) + " dematerializes before your eyes.\n", body);
               body->remove();
          }
...
return(notify_fail("\nError [nuke]: Error while deleting " + file + " in user's
data directory.\n"));
          }
          rmdir(user_data_directory(user));

          write("\nSuccessful [nuke]: User '" + capitalize(user) + "' has been
removed.\n");
