...
void enterWorld(string str)
{
    ...
    if (devp(user))
    {
        if (body->query_env("start_location") == "last_location")
            body->move(body->query("last_location"));
        else if (body->query_env("start_location"))
            body->move(body->query_env("start_location"));
        else if (file_exists(user_path(query_privs(body)) + "workroom.c"))
            body->move(user_path(query_privs(user)) + "workroom.c");
    }
    else body->move( START_LOCATION );

    if (!environment(body)) body->move(START_LOCATION);
