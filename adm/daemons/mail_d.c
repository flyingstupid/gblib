/*
 ...
 Last edited on January 18th, 2007 by Parthenon
...
mapping inbox = ([]);
mapping outbox = ([]);
int curr_in_msg, curr_out_msg, time_to_delete_mail;
int in_end_index, in_start_index, out_end_index, out_start_index;
private object s_editor;
...
mapping restore(string user)
{
    ...
    rtn["time_to_delete_mail"] = time_to_delete_mail;

    return rtn;
}//END restore

void save(string user, mapping in_box, mapping out_box, int time_to_delete, int *indices)
{
    inbox = in_box;
    outbox = out_box;
    time_to_delete_mail = time_to_delete;
