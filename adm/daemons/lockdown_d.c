...
int guest_lock = 0, player_creation_lock = 0;
...
int create()
{
    if(file_exists(SAVE_PATH)) restore_object(SAVE_PATH);
    if(!users_banned) users_banned = ({});
    if(!ips_banned) ips_banned = ({});
    if(!play_testers) play_testers = ({});
}
...
int is_user_banned(string user)
{
    ...
    if(!banned_users || sizeof(banned_users) < 1) return 0;
...
int is_ip_banned(string ip)
{
    ...
    if(!banned_ips || sizeof(banned_ips) < 1)  return 0;
    ...
    for(i = 0; i < sizeof(banned_ips); i++)
    {
        ...
        if(first == "*") first = "[0-9]?[0-9]?[0-9]?";
        if(second == "*") second = "[0-9]?[0-9]?[0-9]?";
        if(third == "*") third = "[0-9]?[0-9]?[0-9]?";
        if(fourth == "*") fourth = "[0-9]?[0-9]?[0-9]?";

        new_ip = "^" + first + "." + second + "." + third + "." + fourth + "$";

        if(ip == curr_ip || regexp(ip, new_ip) == 1) return 1;
    }
...
string query_vip_lock_msg()
{
    if(vip_lock_msg == "") return VIP_LOCK_MSG;
    else return vip_lock_msg;
}

string query_dev_lock_msg()
{
    if(dev_lock_msg == "") return DEV_LOCK_MSG;
    else return dev_lock_msg;
}

string query_guest_lock_msg()
{
    if(guest_lock_msg == "") return GUEST_LOCK_MSG;
    else return guest_lock_msg;
}

string query_player_creation_msg()
{
    if(player_creation_msg == "") return PLAYER_CREATION_MSG;
    else return player_creation_msg;
}

string query_player_lock_msg()
{
    if(player_lock_msg == "") return PLAYER_LOCK_MSG;
    else return player_lock_msg;
}
...
string *query_banned_ips()
{
    ...
    if(!ips_banned)	restore_object(SAVE_PATH);
...
string *query_banned_users()
{
    ...
    if(!users_banned) restore_object(SAVE_PATH);
...
string *query_play_testers()
{
    ...
    if(!play_testers) restore_object(SAVE_PATH);
...
mixed add_banned_user(string name)
{
    ...
    if(!users_banned) users_banned = ({ name });
    else users_banned += ({ name });
...
mixed add_play_tester(string name)
{
    ...
    if(!play_testers) play_testers = ({ name });
    else play_testers += ({ name });
...
void toggle_new_player_creation()
{
    player_creation_lock = !player_creation_lock;
    save_object(SAVE_PATH);
}
