...
void enter_world()
{
    ...
    catch
    {
        ...
        if (news_client->isNew())
            write("\n" + HIG + "Notice:" + NOR + " There are new news
posts.\n\n");
...
void net_dead()
{
    ...
    log_file(LOG_LOGIN, capitalize(query_name()) + " went link-dead on " +
ctime(time()) + "\n");
}
...
void heart_beat()
{
    if(!interactive(this_object()) && !devp(this_object()))
    {
        if((time() - query("last_login")) > 3600)
        {
            ...
            log_file(LOG_LOGIN, capitalize(query_name()) + " auto-quit after 1
hour of net-dead at " + ctime(time()) + ".\n");
    ...
    else
    {
        ...
        if(query_idle(this_object()) % 60 == 0 && query_idle(this_object()) >
300
                && query_env("keepalive") && query_env("keepalive") != "off")
        {
            ...
            keepAliveString = replace_string(keepAliveString, "%t",
ctime(time()));
...
void restore_user()
{
    if(!isMember(query_privs(previous_object() ? previous_object() :
this_player()), "admin") && this_player() != this_object()) return 0;
    if(isMember(query_privs(previous_object()), "admin") ||
query_privs(previous_object()) == this_player()->query_name())
restore_object(user_mob_data(query_name()));
}
...
void add_path(string str)
{
    ...
    {
        write("Error [path]: Directory '" + str + "' is already in your
path.\n");
        return;
    }
...
string query_short()
{
    ...
    if (!interactive(this_object()) && member_array(this_object(), users()) !=
-1)
        str = "A statue of " + str;
