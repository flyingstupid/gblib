...
void setupDefaultObject()
{
    short = file_name( this_object() );
    long = file_name( this_object() );
}
...
void remove()
{
    ...
    allInventory = all_inventory( this_object() );

    if( allInventory )
    {
        foreach (object ob in allInventory)
        {
            if (interactive(ob))
            {
...
{
    return _all_inventory( this_object() );
}
