...
int i_MainMenu(string arg)
{
    ...
    switch(arg)
    {
        case "1":
            ...
            if(LOCKDOWN_D->query_dev_lock())
                write(" 1: Lock developers out (" + HIG + "active" + NOR + ")\n");
            else
                write(" 1: Lock developers out (" + HIG + "inactive" + NOR + ")\n");

            if(LOCKDOWN_D->query_player_lock())
                write(" 2: Lock players out (" + HIG + "active" + NOR + ")\n");
            else
                write(" 2: Lock players out (" + HIG + "inactive" + NOR + ")\n");

            if(LOCKDOWN_D->query_vip_lock())
                write(" 3: Lock mud to all but VIP users (" + HIG + "active" + NOR + ")\n");
            else
                write(" 3: Lock mud to all but VIP users (" + HIG + "inactive" + NOR + ")\n");

            if(LOCKDOWN_D->query_guest_locked())
                write(" 4: Enable guest account\n");
            else
                write(" 4: Disable guest accont\n");

            if(LOCKDOWN_D->query_creation_locked())
                write(" 5: Enable new player creation\n");
            else
                write(" 5: Disable new player creation\n");
    ...
    switch(arg)
    {
        case "1":
            ...
            else
            {
                ...
                {
                    if(i == sizeof(banned_ips) - 1) output += banned_ips[i];
                    else output += banned_ips[i] + ", ";
                }
            }
...
int i_GetIpAddress(string arg, int state)
{
    ...
    if(state == 1) val = LOCKDOWN_D->add_ip_ban(arg);
    else if(state == 2) val = LOCKDOWN_D->remove_ip_ban(arg);
    ...
    {
	    if(state == 1) write(HIG + "\n Success" + NOR 
	        + " [lockdown]: IP address " + first + "." + second + "." + third 
	        + "." + fourth + " successfully banned\n");
	    else if(state == 2) write(HIG + "\n Success" + NOR 
    	    + " [lockdown]: IP address " + first + "." + second + "." + third 
	        + "." + fourth + " successfully unbanned\n");
    }
...
{
    if(!arg || arg == "" || (member_array(lower_case(arg), 
        ({ "n", "y", "no", "yes" })) == -1))
    {
	    write(RED + "\n Error" + NOR + " [lockdown]: invalid option\n");
	    i_MainMenu("2");
...
int i_UserBanChoice(string arg)
{
    ...
    switch(arg)
    {
        case "1":
	        ...
	        if(sizeof(banned_users) == 1) output = capitalize(banned_users[0]);
	        else
	        {
	            for(i = 0; i < sizeof(banned_users); i++)
	            {
		            ...
		            else output += capitalize(banned_users[i]) + ", ";
	            }
	        }
...
varargs int i_GetUserName(string arg, int state, int confirm)
{
    ...
    if(!arg || arg == "")
    {
	    ...
	    if(state == 1) i_UserBanChoice("2");
	    else if(state == 2) i_UserBanChoice("3");
	    return 1;
    }

    tmp = arg;

    if(tmp[0] == '*' && tmp[<1] == '*')	arg = ".*" + arg[1..<2] + ".*";    
    ...
    if(state == 1) val = LOCKDOWN_D->add_banned_user(arg);
    else if(state == 2)	val = LOCKDOWN_D->remove_banned_user(arg);
    ...
    {
	    if(state == 1) write(HIG + "\n Success" + NOR + " [lockdown]: user " 
	        + capitalize(arg) + " successfully banned\n");
	    else if(state == 2) write(HIG + "\n Success" + NOR 
	        + " [lockdown]: user " + capitalize(arg) 
	        + " succesfully unbanned\n");
    }
...
int i_PlayTesterChoice(string arg)
{
    ...
    switch(arg)
    {
        case "1":
	        ...
	        if(sizeof(play_testers) == 1) output = capitalize(play_testers[0]);
	        else
	        {
	            for(i = 0; i < sizeof(play_testers); i++)
	            {
		            ...
		            else output += capitalize(play_testers[i]) + ", ";
	            }
	        }
...
int i_ConRemoveAllPlayTesters(string arg)
{
    ...
    {
	    write(RED + "\n Error" + NOR +
	        " [lockdown]: VIP users could not be removed\n");
	    i_MainMenu("4");
	    return 1;
    }

    write(HIG + "\n Success" + NOR + 
        " [lockdown]: all VIP users have been removed\n");
    i_MainMenu("4");
    return 1;
}
...
varargs int i_GetPlayTester(string arg, int state, int confirm)
{
    ...
    {
	    write(RED + "\n Error" + NOR + " [lockdown]: invalid username\n");
	    if(state == 1) i_PlayTesterChoice("2");
	    else if(state == 2) i_PlayTesterChoice("3");
	    return 1;
    }
    ...
    if(state == 1) val = LOCKDOWN_D->add_play_tester(arg);
    else if(state == 2) val = LOCKDOWN_D->remove_play_tester(arg);
    ...
    {
	    if(state == 1) write(HIG + "\n Success" + NOR + " [lockdown]: user " 
	        + capitalize(arg) + " successfully added as a VIP user\n");
	    else if(state == 2) write(HIG + "\n Success" + NOR 
	        + " [lockdown]: user " + capitalize(arg) 
	        + " succesfully removed as a VIP user\n");
    }
