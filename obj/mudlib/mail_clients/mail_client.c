/*
 ...
 Last edited on January 22nd, 2007 by Parthenon
...
/* GLOBAL VARIABLES */
private string owner, save_path, *recipients, subject, *cc, body, current_file;
private int max_in_msgs, max_out_msgs, did_write, greply, gforward, gflag, date;
mapping inbox;
mapping outbox;
int curr_in_msg, curr_out_msg, in_outbox;
int in_start_index, in_end_index, out_start_index, out_end_index;
int time_to_delete_mail = 20 * 24 * 60 * 60;
object s_editor;

/* FUNCTION PROTOTYPES */
protected void main_menu();
protected int has_new_mail();
...
protected void get_subject(string arg, int flag, int forward);
...
protected void done_reading();
protected void check_old_messages();
...
int start_mail(string arg)
{
    ...
    {
	    destruct(this_object());
	    error(RED + "\nError" + NOR + " [mail]: You must use the mail command "
	        + "to access mail\n");
    }
...
{
    write(HIC + "\n\t" + mud_name() + " Mailer\n\n" + NOR);
    write(" 1: Read mail" + (has_new_mail() ? " " + HIY + "(unread mail) " 
        + NOR + "\n" : "\n"));
...
protected varargs void i_MainMenu(string arg, int flag, int forward)
{
    ...
    switch(arg)
    {
	    ...
	    {
	        write(HIC + "\n\t" + mud_name() + " Mail\n\n" + NOR);
	        write(" 1: Deletion of old mail\n");
	        write(" 2: Main menu\n");
	        input_to("i_WhichOption");
	        return;
	    }
        case "q" :
        case "quit" :
...
protected void i_WhichOption(string arg)
{
    if(!arg || arg == "")
    {
	    i_MainMenu("3");
    ...
    switch(arg)
    {
        case "1" :
	    {
	        if(time_to_delete_mail <= 0 || !time_to_delete_mail)
		        write("Your mail will not be deleted.\n");
	        else
		        write("Currently you mail is set to be deleted after "
		             + HIY + time_to_delete_mail/60/60/24 + NOR + " days.\n");
	        write("\nEnter the new setting or press " + HIW + "[enter]" + NOR 
	            + " to leave it as it is\n(enter 0 for never).\n");
	        input_to("i_ChangeTimeToDelete");
	        return;
	    }
        case "2":
	    {
...
protected void i_ChangeTimeToDelete(string arg)
{
    int time;
    ...
    if(time >= 0) time_to_delete_mail = time * 24 * 60 * 60;
    else
    {
	    write(RED + "\nError" + NOR + " [mail]: Invalid setting\n");
	    i_MainMenu("3");
	    return;
    }
...
protected varargs void do_read_mail()
{
    ...
    if(in_outbox)
    {
	    ...
	    write("\n Displaying " + out_start_index + " to " + out_end_index +
	      " of " + sizeof(outbox) + ".\n");
    }
    else
    {
	    ...
	    for(i = in_start_index; i <= in_end_index; i++)
	    {
	        ...
	        write("\n Displaying " + in_start_index + " to " + in_end_index +
	          " of " + sizeof(inbox) + ".\n");
    }
...
protected varargs void parse_mailcmd(string arg)
{
    ...
    switch(arg)
    {
        ...
        case "del":
	    {
	        ...
	        {
		        write(HIG + "Success" + NOR + " [mail]: Message successfully "+
		          "deleted\n\n");
		        resync_mailbox();
		        prompt();
		        return;
        ...
        case "next":
	    {
	        ...
	        {
		        write(RED + "\nError" + NOR + " [mail]: You have no more " +
		          "messages\n\n");
		        do_read_mail();
		        return;
	        }
        ...
        case "previous":
	    {
	        if((curr_msg - 1) < 1)
	        {
		        write(RED + "\nError" + NOR + " [mail]: There are no previous " +
		          "messages\n\n");
		        do_read_mail();
		        return;
	        }
        ...
        case "forward":
	    {
	        ...
	        input_to("get_recipient", 0, in_outbox, num1);
...
protected varargs void i_SaveDir(string arg, int num1, int num2)
{
    ...
    if(!arg || arg == "")
    {
	    write(RED + "\nError" + NOR + " [mail]: No save file specified\n\n");
	    do_read_mail();
	    return;
    }
    ...
    if(arg[<1] == '/')
    {
	    write(RED + "\nError" + NOR + " [mail]: You must specify a full path " +
	      "including filename\n\n");
	    do_read_mail();
	    return;
    }
    ...
    {
	    write(RED + "\nError" + NOR + " [mail]: You cannot save there, that " +
	      "is a directory name.\n\n");
	    do_read_mail();
	    return;
    }
    ...
    {
	    write(HIG + "\nSuccess" + NOR + " [mail]: Message successfully saved " +
	      "to " + path + "\n\n");
	    do_read_mail();
    }
}
...
protected varargs void i_Overwrite(string arg, string path, int num1, int num2)
{
    ...
    if(lower_case(arg) == "y" || lower_case(arg) == "n")
    {
	    ...
	    {
	        write(HIG + "\nSuccess" + NOR + " [mail]: Message successfully " +
	          "saved to " + path + "\n\n");
	        do_read_mail();
	        return;
	    }
...
protected void read_message(int num)
{
    ...
    if(in_outbox)
    {
	    ...
	    ret += "DATE:    " + ctime(outbox[num]["DATE"]) + "\n";
...
protected varargs void do_mail(string arg, int flag, int forward)
{
    ...
    for(i = 0; i < sizeof(recipients); i++)
    {
	    if(!file_exists(user_data_file(recipients[i])+ ".o"))
	    {
	        ...
	        {
		        if(member_array(recipients[i][1..<2], s_editor->listGroups())
		            == -1)
		        {
...
void callback_exit()
{
    ...
    if(greply)
    {
	    ...
	    body += "> DATE: " + (gflag ? ctime(outbox[greply]["DATE"]) + "\n" :
	      ctime(inbox[greply]["DATE"]) + "\n");
    ...
    else if(gforward)
    {
	    ...
	    body += "> DATE: " + (gflag ? ctime(outbox[gforward]["DATE"]) + "\n" :
	      ctime(inbox[gforward]["DATE"]) + "\n");
...
protected void resync_mailbox()
{
    ...
    if(file_exists(save_path))
    {
	    ...
	    time_to_delete_mail = vars["time_to_delete_mail"];
...
{
    int i, in_index = 1, out_index = 1;
    int msg_date, curr_date;
    mapping tmp_inbox = ([]);
    mapping tmp_outbox = ([]);

    if(!time_to_delete_mail) time_to_delete_mail = 0;
        
    if(time_to_delete_mail <= 0) return;

    curr_date = time();

    for(i = 1; i <= sizeof(inbox); i++)
    {
	    msg_date = inbox[i]["DATE"];

	    if((curr_date - msg_date) < time_to_delete_mail)
	        tmp_inbox[in_index++] = inbox[i];
    }

    for(i = 1; i <= sizeof(outbox); i++)
    {
	    msg_date = outbox[i]["DATE"];

	    if((curr_date - msg_date) < time_to_delete_mail)
	        tmp_outbox[out_index++] = outbox[i];
    }
...
protected void save_mailbox()
{
    MAIL_D->save(owner, inbox, outbox, time_to_delete_mail, 
        ({ curr_in_msg, curr_out_msg, in_start_index, in_end_index, 
...
protected mixed save_message(string path, mapping message)
{
    ...
    if(sizeof(message) == 1)
    {
	    ...
	    text += "DATE:    " + ctime(message[0]["DATE"]) + "\n";
    ...
    else
    {
	    for(i = 0; i < sizeof(message); i++)
	    {
	        ...
	        text += "DATE:    " + ctime(message[i]["DATE"]) + "\n";
